...
export function MiniMapOverlayGrid({ apiKey }: { apiKey: string }) {
  ...
  const activeMiniMaps = miniMaps.filter(
    (m) => visibleMiniMapIds.includes(m.id) && m.id !== focusedMiniMapId,
  );
      ...
      <AnimatePresence mode="popLayout">
        ...
        {activeMiniMaps.map((map) => (
          ...
          >
            <FleetMap
              apiKey={apiKey}
              trackedVehicleIds={map.vehicleIds}
              isMainMap={false}
            />
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      onClick={() =>
                        dispatch({ type: "FOCUS_MINIMAP", payload: map.id })
                      }
                    >
                  ...
                  <TooltipContent side="left">
                    Intercambiar con Mapa Principal
                  </TooltipContent>
                </Tooltip>
