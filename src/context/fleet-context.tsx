...
const FleetStateContext = createContext<FleetStateContextValue | undefined>(
  undefined,
);
const FleetDispatchContext = createContext<Dispatch<FleetAction> | undefined>(
  undefined,
);
...
export const FleetProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (Math.random() > 0.95) {
        const randomVehicle =
          state.vehicles[Math.floor(Math.random() * state.vehicles.length)];
